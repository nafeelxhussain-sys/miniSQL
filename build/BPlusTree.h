...
class BplusTree{
    ...
    int find_in_leaf(Disk_Manager &dm,const int &pageid ,datatype dt,const void *key,
        const int &key_size,const int &row_size, const int &key_off);

    void insert_leaf(Disk_Manager &dm,datatype dt, const void* row,
        const int &key_size,const int &row_size, const int &key_off);
    ...
    void insert_parent(Disk_Manager &dm,datatype dt,const int &parentId ,
        const void *key,const int &key_size,const int &childpointer);

    SplitInfo parent_split(Disk_Manager &dm,Table_Metadata &tmd,Page &left_pg,const void * key,const int &key_size,
    ...
    void create_new_root(Disk_Manager &dm,Table_Metadata &tmd,const int &leaf_id,
