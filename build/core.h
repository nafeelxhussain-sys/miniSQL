...
class catalog {
    ...
    uint16_t coulumn_counts[MAX_TABLES];
    uint8_t table_clustered[MAX_TABLES];
    int table_count;
...
public:
    ...
    DB_error add_table(string table_name, uint16_t coulumn_count ,uint8_t is_clustered=0);
...
public:
    ...
    DB_error create_table(string table_name, int num_of_cols, string* name, int* size, datatype* type, int * col_index);
